...
def _make_explained(item: MediaItem, rank: int, rv: float,
                    reasons=None, matched_tags=None) -> ExplainedResult:
...
class TestToJson:
    ...
    def test_json_contains_required_keys(self, cfg, scored_candidates):
        ...
        row = data[0]
        for key in ("rank", "title", "type", "year", "rating", "recommendation_value",
                    "component_scores", "reasons", "matched_tags", "genres", "tags"):
