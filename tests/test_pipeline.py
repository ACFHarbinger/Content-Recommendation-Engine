...
All external dependencies (Qdrant, BGE-M3, Anthropic) are replaced with
the mock fixtures from conftest.py.
"""
...
def _make_pipeline(qdrant_mock, cfg, mock_embedder, explain=False, use_reranker=False):
...
def _populate(qdrant_mock, cfg, mock_embedder, items):
    from src.store import QdrantStore
    store = QdrantStore(cfg)
...
class TestPipelineRun:
    def test_returns_explained_results(
        ...
        _populate(qdrant_mock, cfg, mock_embedder, sample_items)
        pipeline = _make_pipeline(qdrant_mock, cfg, mock_embedder)
    ...
    def test_ranks_start_at_one(self, qdrant_mock, cfg, mock_embedder, sample_items):
        _populate(qdrant_mock, cfg, mock_embedder, sample_items)
        pipeline = _make_pipeline(qdrant_mock, cfg, mock_embedder)
        results = pipeline.run("any query")
        if results:
    ...
    def test_top_k_limits_output(self, qdrant_mock, cfg, mock_embedder, sample_items):
        _populate(qdrant_mock, cfg, mock_embedder, sample_items)
        from src.pipeline import RecommendationPipeline
        pipeline = _make_pipeline(qdrant_mock, cfg, mock_embedder)
        pipeline._top_k = 2
    ...
    def test_no_explain_produces_empty_reasons(
        ...
        _populate(qdrant_mock, cfg, mock_embedder, sample_items)
        pipeline = _make_pipeline(qdrant_mock, cfg, mock_embedder, explain=False)
        results = pipeline.run("any query")
        for r in results:
            assert r.reasons == []
    ...
    def test_empty_collection_returns_empty_list(
        self, qdrant_mock, cfg, mock_embedder
    ):
        pipeline = _make_pipeline(qdrant_mock, cfg, mock_embedder)
        results = pipeline.run("any query")
        assert results == []
